    ...
    <div x-data="{ isLogin: true }" class="min-h-screen flex flex-col-reverse lg:flex-row bg-brand-background font-sans text-gray-900">
        ...
        <div class="w-full lg:w-1/2 flex flex-col justify-center px-8 sm:px-16 lg:px-24 py-12 lg:py-0">
            <div class="max-w-md w-full mx-auto">
                ...
                <!-- LOGIN FORM -->
                <form x-show="isLogin" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                    <div>
                        <label class="block text-sm font-semibold text-black mb-1">Email Address</label>
                        <input type="email" class="w-full px-0 py-2 bg-transparent border-0 border-b-2 border-gray-300 focus:ring-0 focus:border-brand-blue transition-colors outline-none" placeholder="owner@example.com">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-black mb-1">Password</label>
                        <input type="password" class="w-full px-0 py-2 bg-transparent border-0 border-b-2 border-gray-300 focus:ring-0 focus:border-brand-blue transition-colors outline-none" placeholder="••••••••">
                    </div>
                    
                    <button type="button" class="w-full bg-brand-blue hover:bg-brand-blue-hover text-white font-bold py-3 px-4 rounded-lg transition-colors mt-4 shadow-lg shadow-brand-blue/30">
                        Log In
                    </button>
                ...
                <!-- SIGN UP FORM -->
                <form x-show="!isLogin" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                    <div>
                        <label class="block text-sm font-semibold text-black mb-1">Full Name</label>
                        <input type="text" class="w-full px-0 py-2 bg-transparent border-0 border-b-2 border-gray-300 focus:ring-0 focus:border-brand-blue transition-colors outline-none" placeholder="Juan Dela Cruz">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-black mb-1">Email Address</label>
                        <input type="email" class="w-full px-0 py-2 bg-transparent border-0 border-b-2 border-gray-300 focus:ring-0 focus:border-brand-blue transition-colors outline-none" placeholder="owner@example.com">
...
<div class="w-full lg:w-1/2 p-4 lg:p-8 flex items-center justify-center h-[40vh] lg:h-auto">
    ...
    <div x-data="{
        ...
        <!-- Gradient Overlay and Text (Pointer-events-none prevents it from blocking arrow clicks) -->
        ...
        <!-- Manual Navigation Arrows (Hidden on mobile to save space, visible on hover for desktop) -->
