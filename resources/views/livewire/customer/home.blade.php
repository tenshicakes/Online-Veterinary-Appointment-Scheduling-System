    ...
    <!-- PAGE HEADER -->
    <div>
        <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Welcome, {{ auth()->user()->name ?? 'Pet Owner' }}!</h1>
    ...
    <div class="bg-brand-blue rounded-3xl p-6 md:p-8 shadow-xl text-white relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <span class="inline-block px-3 py-1 bg-white/20 text-blue-100 text-md font-bold uppercase tracking-wider rounded-full mb-3">
                    Next Appointment
                </span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <a href="/appointment" class="flex items-center justify-center p-4 bg-brand-blue hover:bg-brand-blue-hover text-white font-bold rounded-2xl shadow-md transition-all group">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-brand-blue text-md uppercase tracking-wider text-white border-b border-gray-100">
                    ...
                    @foreach($appointmentHistory as $apt)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-md font-bold text-gray-900">{{ \Carbon\Carbon::parse($apt['date'])->format('M j, Y') }}</div>
                                <div class="text-md font-medium text-gray-500">{{ $apt['time'] }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($apt['status'] === 'Confirmed')
                                    <span class="px-2.5 py-1 inline-flex text-md leading-5 font-bold rounded-full text-green-700">Confirmed</span>
                                @elseif($apt['status'] === 'Completed')
                                    <span class="px-2.5 py-1 inline-flex text-md leading-5 font-bold rounded-full text-gray-600">Completed</span>
                                @elseif($apt['status'] === 'Pending')
                                    <span class="px-2.5 py-1 inline-flex text-md leading-5 font-bold rounded-full text-blue-600">Pending</span>
                                @else
                                    <span class="px-2.5 py-1 inline-flex text-md leading-5 font-bold rounded-full text-red-600">Canceled</span>
                                @endif
                            </td>
                        </tr>
                    @endforeach
